...
import { GazeDirection, GAZE_DESCRIPTIONS } from "./GazeDirector";
...
export default function BiometricsDashboard({ basePrompt, gazeDirection, landmarkAnalysis, isProcessing }: BiometricsDashboardProps) {
  const [copied, setCopied] = useState(false);

  // Compute final real-time responsive prompt
  // Suffix the gaze direction clause to the prompt string elegantly!
  const gazeDescriptor = GAZE_DESCRIPTIONS[gazeDirection]?.text || "";
  const finalPrompt = basePrompt 
    ? (basePrompt.endsWith(",") || basePrompt.endsWith(".") 
        ? `${basePrompt.slice(0, -1)}, with ${gazeDescriptor}` 
        : `${basePrompt}, with ${gazeDescriptor}`) 
    : "";
      ...
      <div id="prompt-output-card" className="rounded-3xl border-4 border-slate-950 bg-[#2a2929] p-6 shadow-[6px_6px_0px_#000] space-y-5">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between border-b-2 border-slate-900 pb-4 gap-4">
          ...
          <button
            id="copy-prompt-btn"
            onClick={handleCopy}
            disabled={!finalPrompt || isProcessing}
        ...
        <div className="relative p-5 rounded-2xl bg-[#1f1e1e] border-3 border-slate-950 shadow-[inner_0_2px_4px_rgba(0,0,0,0.5)]">
          <p className="text-sm font-mono text-amber-200 leading-relaxed whitespace-pre-wrap select-all font-bold">
            {finalPrompt || "Put a silly selfie photo on the scanner to see the magic happen!"}
          </p>
