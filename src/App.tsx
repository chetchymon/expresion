...
import GazeDirector, { GazeDirection } from "./components/GazeDirector";
...
export default function App() {
  ...
  const handleFaceDetected = async (imageSrc: string, mimeType: string, analysis: LandmarkAnalysis) => {
    ...
    try {
      ...
      if (!response.ok) {
        let errText = "Magic spell synthesis failed.";
        try {
          ...
          if (contentType && contentType.includes("application/json")) {
            ...
            if (response.status === 404) {
              errText = "API endpoint not found (Error 404). Static hosting platforms like Netlify only host static files and do not run the custom Node/Express backend (/api/analyze) required to communicate securely with the Gemini API. For the scanner to work, please run the app locally using 'npm run dev', or deploy to a full-stack container host (like Cloud Run).";
            } else {
              errText = `Server returned status ${response.status}.`;
            }
          }
        } catch (e) {
          if (response.status === 404) {
            errText = "API endpoint not found (Error 404). Netlify is a static-only provider and cannot run our Express backend. To use the facial recognition magic, run this applet locally, or deploy it to a full-stack Node container.";
          } else {
            errText = `Connection failed with status ${response.status}.`;
          }
        }
      ...
      try {
        ...
        throw new Error("The server responded, but the response couldn't be parsed. Please check that your full-stack Node express server is running.");
      }
    ...
    <div className="min-h-screen bg-[#1f1e1e] text-[#f4f4f3] flex flex-col font-sans selection:bg-amber-400/40 selection:text-amber-200">
      ...
      <main className="flex-grow max-w-6xl w-full mx-auto px-4 md:px-8 py-8 z-10 grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div className="lg:col-span-6 space-y-6">
          ...
          {/* Eye Look-O-Matic Selection Panel */}
          <GazeDirector value={gazeDirection} onChange={setGazeDirection} />
